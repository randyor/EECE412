...
client_shared_secret_num = 0 #initial value 0
server_shared_secret_num = 0 #initial value 0
def sxor(s1,s2):
...
class eece412A2(Tkinter.Tk):
	...
	b = random.randint(0,10000)		#This is b's nonce for DH session key
	Rb = random.randint(0,10000000) 	#This is also a nonce
	server_name = 'Bob'
	...
	while 1:
		...
		if received_Rb == str(Rb):
			...
			print "Transmission received: ", received_cipher
		        self.exTraInfoMessage.set("Transmision received: " + received_cipher)
		        self.update()
			os.system("pause")
			cipher_message, mac_received, received_gar= received_cipher.split(",")
			print "cipher received: ", received_cipher
			print "cipher received: ", mac_received
		        self.exTraInfoMessage.set("cipher received: " + received_cipher)
		        self.update()
			os.system("pause")
			self.exTraInfoMessage.set("mac received: " + mac_received)
		        self.update()
			os.system("pause")
			obj_sha_2 = SHA.new()
			mac_2 = obj_sha_2.update(sxor(received_cipher, server_shared_secret_num))
			if mac_2 == mac_received :
				temp_key = self.UserKey.get()
				temp_x = temp_key[int(server_shared_secret_num)]
				temp_key[int(server_shared_secret_num)] = temp_key[15 - int(server_shared_secret_num)]
				temp_key = temp_key[: (15 - int(client_shared_secret_num) - 1) ] + temp_x + temp_key[ (15 - int(client_shared_secret_num) ):]
				print "key computed from symmetric key and secret share number, length: ", temp_key, len(temp_key)
				obj2 = AES.new(temp_key, AES.MODE_CBC, 'This is an IV456')
				plaintext = obj2.decrypt(received_cipher)
				plaintext, gar_3 = plaintext.split("^", 1)
				if not plaintext: break
				print "plaintext:", plaintext
			else :
				print "MAC verification failed, message is corrupted!!"
	...
	a = random.randint(0,10000) 		#This is a's nonce for DH session key
	Ra = random.randint(0,10000000)  	#This is also a nonce
	client_name = 'alice'
	...
	if clearLen_2%16 > 0 :
		...
		for x in range (1, (16 - clearLen_2%16)):
		    message = message +'0'
	if client_shared_secret_num == '0':
		client_shared_secret_num = '7'
	temp_key_1 = self.UserKey.get()
	temp_x_1 = temp_key_1[int(client_shared_secret_num)]
	temp_key_1[int(client_shared_secret_num)] = temp_key_1[15 - int(client_shared_secret_num)]
	temp_key_1 = temp_key_1[: (15 - int(client_shared_secret_num) - 1) ] + temp_x_1 + temp_key_1[ (15 - int(client_shared_secret_num) ):]
	print "key computed from symmetric key and secret share number, length: ", temp_key_1, len(temp_key_1)
	obj = AES.new(temp_key_1, AES.MODE_CBC, 'This is an IV456')
	ciphertext = obj.encrypt(message)
	obj_sha_1 = SHA.new()
	mac = obj_sha_1.update(sxor(ciphertext, client_shared_secret_num))
	...
	self.connectionSocket.send(ciphertext + ',' + mac + ',')
	print "cipher and mac sent: ", ciphertext, mac
